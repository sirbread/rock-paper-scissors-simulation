...
paper_icon = pygame.image.load('assets/paper.png')
...
running = True
paused = False  # Track whether the simulation is paused
...
while running:
    ...
    if not paused:
        adjust_movement()  # Move and draw entities when not paused
    else:
        # Draw entities in their last positions without movement
        for entity in entities:
            entity.draw()
            
        # Display pause overlay
        pause_text_surface = font_large.render("Paused", True, pygame.Color("#ffffff"))
        box_width = pause_text_surface.get_width() + 40
        box_height = pause_text_surface.get_height() + 40

        pygame.draw.rect(screen, (0, 0, 0), 
                         (WINDOW_WIDTH // 2 - box_width // 2, WINDOW_HEIGHT // 2 - box_height // 2, box_width, box_height))
        screen.blit(pause_text_surface, 
                    (WINDOW_WIDTH // 2 - pause_text_surface.get_width() // 2, WINDOW_HEIGHT // 2 - pause_text_surface.get_height() // 2))

    # Count entities of each tribe to check for a winner
